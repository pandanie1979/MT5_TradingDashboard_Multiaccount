...
def render(accounts_data: dict):
    st.header("Impostazioni & Configurazione")
            ...
            with col2:
                ...
                if st.button(f"Test Connessione Account {selected_account}", use_container_width=True):
                    with st.spinner("Verificando connessione..."):
                        ...
                        if success:
                            st.success(f"[OK] Connessione OK per Account {selected_account}")
                            st.json(result)
                        else:
                            st.error(f"[ERR] Errore connessione: {result}")
            ...
            if is_valid:
                ...
                if new_path not in current_paths:
                    ...
                    if save_accounts_config(updated_paths):
                        st.success(f"[OK] Percorso aggiunto: {new_path}. Ricarica la pagina per vedere le modifiche.")
                        # RIMOSSO st.rerun() per preservare tab
                    else:
                        st.error("[ERR] Errore nel salvare la configurazione")
                else:
                    st.warning("⚠ Percorso gia presente nella configurazione")
            else:
                st.error(f"[ERR] Percorso non valido: {message}")
...
def render_debug_info(accounts_data: dict):
    ...
    with col1:
        ...
        st.metric("File Configurazione", "[OK] Presente" if config_exists else "[ERR] Mancante")
    ...
    with col2:
        ...
        # Verifica disponibilita cache
        cache_available = hasattr(st.cache_data, 'clear')
        st.write(f"Cache Streamlit: {'[OK] Disponibile' if cache_available else '[ERR] Non disponibile'}")
                ...
                with col2:
                    ...
                    if st.button(f"Test {account_id}", key=f"debug_test_{account_id}"):
                        with st.spinner(f"Testing Account {account_id}..."):
                            ...
                            if success:
                                st.success("[OK] Connessione OK")
                                st.json(result)
                            else:
                                st.error(f"[ERR] Errore: {result}")
    ...
    st.markdown("---")
    st.subheader("Tools di Manutenzione")
    ...
    with col3:
        if st.button("Reset Completo", use_container_width=True):
            if st.button("CONFERMA RESET", key="confirm_reset", type="primary"):
                # Reset configurazione
                if save_accounts_config(DEFAULT_MT5_PATHS):
                    st.cache_data.clear()
                    st.success("[OK] Reset completato! Ricarica la pagina manualmente per vedere le modifiche.")
                    # RIMOSSO st.rerun() - ultimo caso risolto
                else:
                    st.error("[ERR] Errore durante il reset")
...
def scan_path_for_accounts(path: str):
    ...
    if not os.path.exists(path):
        st.error("[ERR] Percorso non trovato")
        return

    accounts = get_available_accounts_from_path(path)

    if accounts:
        st.success(f"[OK] Trovati {len(accounts)} account: {', '.join(accounts)}")
        ...
        st.warning("⚠ Nessun account trovato in questo percorso")
