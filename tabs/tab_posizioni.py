...
def render(account_id: str, account_path: str, account_info: dict):
    ...
    if ea_df is None or ea_df.empty:
        st.error(f"[Errore] Impossibile caricare i dati degli EA per Account {account_id}")
    ...
    styled_df = table_df.style.map(color_margin, subset=[margin_col]) \
    ...
    st.markdown("---")
    st.subheader(f"⚠ Monitoraggio Posizioni Account {account_id}")

    # Alert per tick non ricevuti (piu di 5 minuti)
    old_ticks = ea_with_positions[ea_with_positions['Minutes_Since_Last_Tick'] > 5]
    if not old_ticks.empty:
        st.warning(f"⚠ {len(old_ticks)} posizioni non ricevono tick da piu di 5 minuti:")
        for _, pos in old_ticks.iterrows():
            st.write(f"- **{pos['Setup_Name']}** ({pos['Symbol']}): {pos['Minutes_Since_Last_Tick']:.1f} minuti fa")
    else:
        st.success(f"[OK] Tutte le posizioni di Account {account_id} ricevono tick regolarmente")
    ...
    if not inactive_with_positions.empty:
        ...
        for _, ea in inactive_with_positions.iterrows():
            st.write(
                f"- **{ea['Setup_Name']}** ({ea['Symbol']}): "
                f"Status = {ea['Status']}, Margine = €{ea['Position_Margin']:.2f}"
            )
    ...
    symbol_summary.columns = [f'Margine Totale (€)', 'Num. Posizioni', f'Margine Medio (€)']
    symbol_summary = symbol_summary.sort_values(f'Margine Totale (€)', ascending=False)
    ...
    if 'EA_Start_Time' in ea_with_positions.columns:
        ...
        if not ea_starts.empty:
            ...
            for _, row in ea_starts.head(10).iterrows():
                ...
                st.write(
                    f"- **{row['Setup_Name']}** ({row['Symbol']}): "
                    f"€{row['Position_Margin']:.2f} - {time_str}"
                )
        else:
    ...
    with col1:
        if st.button(f"Aggiorna Posizioni Account {account_id}", use_container_width=True):
            st.cache_data.clear()
            st.success("[OK] Cache pulita! I dati posizioni saranno aggiornati automaticamente.")

    with col2:
        st.write("")

    # Riepilogo finale
    status_label = "[OK]" if len(old_ticks) == 0 and len(inactive_with_positions) == 0 else "[Attenzione]"
    st.markdown(f"""
    <div style="background: linear-gradient(90deg, {account_color}22 0%, {account_color}11 100%);
        ...
        &bull; {len(ea_with_positions)} posizioni aperte per un margine totale di €{total_margin:.2f}<br>
        &bull; {len(ea_with_positions[ea_with_positions['Status'] == 'ACTIVE'])} EA attivi con posizioni<br>
        &bull; Symbols coinvolti: {', '.join(ea_with_positions['Symbol'].unique())}<br>
        &bull; Status monitoraggio: {status_label}
    </div>
    """, unsafe_allow_html=True)
